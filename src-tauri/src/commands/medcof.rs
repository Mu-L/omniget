...
use crate::platforms::medcof::api::{self, MedcofCourse, MedcofSession};
use crate::platforms::medcof::downloader;
use crate::AppState;
...
struct MedcofDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn medcof_login_token(
    state: tauri::State<'_, AppState>,
    token: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.medcof_session.lock().await.take();
    ...
    *state.medcof_courses_cache.lock().await = None;
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.medcof_session.lock().await;
            *guard = Some(session);
            *state.medcof_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn medcof_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.medcof_session.lock().await.is_some();
    ...
    let guard = state.medcof_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.medcof_session_validated_at.lock().await;
...
async fn fetch_medcof_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<MedcofCourse>, String> {
    let guard = state.medcof_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.medcof_courses_cache.lock().await;
    *cache = Some(crate::MedcofCoursesCache {
...
#[tauri::command]
pub async fn medcof_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<MedcofCourse>, String> {
    {
        let cache = state.medcof_courses_cache.lock().await;
    ...
    fetch_medcof_courses(&state).await
}

#[tauri::command]
pub async fn medcof_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<MedcofCourse>, String> {
    {
        let mut cache = state.medcof_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_medcof_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: MedcofCourse =
