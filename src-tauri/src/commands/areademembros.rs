...
use crate::platforms::areademembros::api::{self, AreaDeMembrosApiCourse, AreaDeMembrosSession};
use crate::platforms::areademembros::downloader;
use crate::AppState;
...
struct AreaDeMembrosDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn areademembros_login_token(
    state: tauri::State<'_, AppState>,
    cookies: String,
    site_url: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.areademembros_session.lock().await.take();
    *state.areademembros_session_validated_at.lock().await = None;
    *state.areademembros_courses_cache.lock().await = None;

    let clean_url = site_url.trim_end_matches('/').to_string();

    let session = AreaDeMembrosSession {
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.areademembros_session.lock().await;
            *guard = Some(session);
            *state.areademembros_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn areademembros_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.areademembros_session.lock().await.is_some();
    ...
    let guard = state.areademembros_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.areademembros_session_validated_at.lock().await;
...
#[tauri::command]
pub async fn areademembros_logout(
    state: tauri::State<'_, AppState>,
) -> Result<(), String> {
    let _ = api::delete_saved_session().await;
    state.areademembros_session.lock().await.take();
...
async fn fetch_areademembros_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<AreaDeMembrosApiCourse>, String> {
    let guard = state.areademembros_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.areademembros_courses_cache.lock().await;
    *cache = Some(crate::AreaDeMembrosCoursesCache {
...
#[tauri::command]
pub async fn areademembros_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<AreaDeMembrosApiCourse>, String> {
    {
        let cache = state.areademembros_courses_cache.lock().await;
    ...
    fetch_areademembros_courses(&state).await
}

#[tauri::command]
pub async fn areademembros_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<AreaDeMembrosApiCourse>, String> {
    {
        let mut cache = state.areademembros_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_areademembros_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: AreaDeMembrosApiCourse =
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(()) => {
                let _ = app.emit(
                    "areademembros-download-complete",
                    &AreaDeMembrosDownloadCompleteEvent {
            ...
            Err(e) => {
                tracing::error!("[areademembros] download error for '{}': {}", course.name, e);
                let _ = app.emit(
                    "areademembros-download-complete",
                    &AreaDeMembrosDownloadCompleteEvent {
                        course_name: course.name,
