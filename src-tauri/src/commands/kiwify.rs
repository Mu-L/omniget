...
struct KiwifyDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
pub async fn kiwify_login(
    ...
    state.kiwify_session.lock().await.take();
    *state.kiwify_session_validated_at.lock().await = None;
    *state.kiwify_courses_cache.lock().await = None;

    match api::authenticate(&email, &password).await {
        Ok(session) => {
            let response_email = session.email.clone();
            let _ = api::save_session(&session).await;
            let mut guard = state.kiwify_session.lock().await;
            *guard = Some(session);
            *state.kiwify_session_validated_at.lock().await = Some(Instant::now());
            Ok(response_email)
        }
        Err(e) => {
            tracing::error!("[kiwify] login failed: {}", e);
...
pub async fn kiwify_check_session(
    ...
    let has_memory_session = state.kiwify_session.lock().await.is_some();
    ...
    {
        let validated_at = state.kiwify_session_validated_at.lock().await;
...
pub async fn kiwify_list_courses(
    ...
    {
        let cache = state.kiwify_courses_cache.lock().await;
    ...
    fetch_kiwify_courses(&state).await
}
