...
use crate::platforms::entregadigital::api::{self, EntregaDigitalCourse, EntregaDigitalSession};
use crate::platforms::entregadigital::downloader;
use crate::AppState;
...
struct EntregaDigitalDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn entregadigital_login_token(
    ...
    state.entregadigital_session.lock().await.take();
    ...
    *state.entregadigital_courses_cache.lock().await = None;
    ...
    let session = EntregaDigitalSession {
        ...
        client: crate::core::http_client::apply_global_proxy(reqwest::Client::builder())
            ...
            .default_headers({
                ...
                h.insert("os", os_value.parse().unwrap());
                h
            })
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.entregadigital_session.lock().await;
            *guard = Some(session);
            *state.entregadigital_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn entregadigital_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.entregadigital_session.lock().await.is_some();
    ...
    let guard = state.entregadigital_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.entregadigital_session_validated_at.lock().await;
...
#[tauri::command]
pub async fn entregadigital_logout(
    state: tauri::State<'_, AppState>,
) -> Result<(), String> {
    let _ = api::delete_saved_session().await;
    state.entregadigital_session.lock().await.take();
...
async fn fetch_entregadigital_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<EntregaDigitalCourse>, String> {
    let guard = state.entregadigital_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.entregadigital_courses_cache.lock().await;
    *cache = Some(crate::EntregaDigitalCoursesCache {
...
#[tauri::command]
pub async fn entregadigital_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<EntregaDigitalCourse>, String> {
    {
        let cache = state.entregadigital_courses_cache.lock().await;
    ...
    fetch_entregadigital_courses(&state).await
}

#[tauri::command]
pub async fn entregadigital_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<EntregaDigitalCourse>, String> {
    {
        let mut cache = state.entregadigital_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_entregadigital_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: EntregaDigitalCourse =
