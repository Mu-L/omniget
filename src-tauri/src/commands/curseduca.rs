...
use crate::platforms::curseduca::api::{self, CurseducaCourse, CurseducaSession};
use crate::platforms::curseduca::downloader;
use crate::AppState;
...
struct CurseducaDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
pub async fn curseduca_login(
    ...
    state.curseduca_session.lock().await.take();
    *state.curseduca_session_validated_at.lock().await = None;
    *state.curseduca_courses_cache.lock().await = None;

    let clean_url = site_url.trim_end_matches('/').to_string();

    match api::authenticate(&clean_url, &username, &password).await {
        Ok(session) => {
            let _ = api::save_session(&session).await;
            let mut guard = state.curseduca_session.lock().await;
            *guard = Some(session);
            *state.curseduca_session_validated_at.lock().await = Some(Instant::now());
            Ok("authenticated".to_string())
        }
        Err(e) => {
            tracing::error!("[curseduca] login failed: {}", e);
...
#[tauri::command]
pub async fn curseduca_login_token(
    state: tauri::State<'_, AppState>,
    token: String,
    api_key: String,
    site_url: String,
    ...
    let session = CurseducaSession {
        ...
        client: crate::core::http_client::apply_global_proxy(reqwest::Client::builder())
            ...
            .default_headers({
                ...
                h.insert("api_key", api_key.parse().unwrap());
...
#[tauri::command]
pub async fn curseduca_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.curseduca_session.lock().await.is_some();
    ...
    let guard = state.curseduca_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.curseduca_session_validated_at.lock().await;
...
async fn fetch_curseduca_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<CurseducaCourse>, String> {
    let guard = state.curseduca_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.curseduca_courses_cache.lock().await;
    *cache = Some(crate::CurseducaCoursesCache {
...
#[tauri::command]
pub async fn curseduca_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<CurseducaCourse>, String> {
    {
        let cache = state.curseduca_courses_cache.lock().await;
    ...
    fetch_curseduca_courses(&state).await
}

#[tauri::command]
pub async fn curseduca_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<CurseducaCourse>, String> {
    {
        let mut cache = state.curseduca_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_curseduca_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: CurseducaCourse =
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(()) => {
                let _ = app.emit(
                    "curseduca-download-complete",
                    &CurseducaDownloadCompleteEvent {
            ...
            Err(e) => {
                tracing::error!("[curseduca] download error for '{}': {}", course.name, e);
                let _ = app.emit(
                    "curseduca-download-complete",
                    &CurseducaDownloadCompleteEvent {
                        course_name: course.name,
