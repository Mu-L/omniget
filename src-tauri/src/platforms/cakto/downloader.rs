...
use super::api::{self, CaktoCourse, CaktoSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct CaktoDownloadProgress {
...
pub async fn download_full_course(
    ...
    if crate::core::course_utils::is_course_complete(&course_dir) {
        tracing::info!("[cakto] course already complete, skipping");
        return Ok(());
    }
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
                ...
                if !video_url.is_empty() {
                    ...
                    if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                        ...
                        if meta.map(|m| m.len() > 0).unwrap_or(false) {
                            tracing::info!("[cakto] Skipping existing: {}", video_path);
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[cakto] yt-dlp download failed for '{}': {}, trying direct",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!("[cakto] Direct download also failed for '{}': {}", lesson.name, e2);
                                }
                            }
                        }
                    }
            ...
            for file in &lesson.files {
                ...
                let download_url = if let Some(ref url) = file.url {
                    ...
                    match api::get_file_download_url(session, &lesson.id, &file.id).await {
                        Ok(url) => url,
                        Err(e) => {
                            tracing::error!("[cakto] Failed to get download URL for file '{}': {}", file.name, e);
                            continue;
                        }
                    }
                };
                ...
                match download_file_direct(&session.client, &download_url, &file_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!("[cakto] File download failed for '{}': {}", file.name, e);
                    }
                }
            }
