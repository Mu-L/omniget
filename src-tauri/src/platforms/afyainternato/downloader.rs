...
use super::api::{self, AfyaCourse, AfyaSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct AfyaCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let modules = api::get_modules(session, &course.product_id).await?;
    ...
    let _ = app.emit(
        "afya-download-progress",
        &AfyaCourseDownloadProgress {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            if lesson.type_id == "videoClass" || lesson.type_id.contains("video") {
                match api::get_video_url(session, &lesson.id, &lesson.product_id).await {
                    Ok(video_url) => {
                        ...
                        if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                            ...
                            if meta.map(|m| m.len() > 0).unwrap_or(false) {
                                tracing::info!("[afya] Skipping existing: {}", video_path);
                        ...
                        if video_url.contains(".m3u8") {
                            ...
                            match download_with_ytdlp(&video_url, &lesson_dir, &cancel_token).await {
                                ...
                                Err(e) => {
                                    tracing::error!(
                                        "[afya] yt-dlp download failed for '{}': {}, trying direct download",
                                        lesson.name,
                                        e
                                    );
                                    match download_file_direct(&session.client, &video_url, &video_path, &cancel_token).await {
                                        ...
                                        Err(e2) => {
                                            tracing::error!(
                                                "[afya] Direct download also failed for '{}': {}",
                                                lesson.name,
                                                e2
                                            );
                                        }
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[afya] Failed to get video URL for '{}': {}",
                            lesson.name,
                            e
                        );
                    }
            ...
            for material in &lesson.materials {
                ...
                let ext = material.url.rsplit('.').next().unwrap_or("pdf");
                let ext = ext.split('?').next().unwrap_or(ext);
                let mat_name = filename::sanitize_path_component(&material.title);
                let mat_path = format!("{}/{}.{}", lesson_dir, mat_name, ext);

                if tokio::fs::try_exists(&mat_path).await.unwrap_or(false) {
                    let meta = tokio::fs::metadata(&mat_path).await;
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        continue;
                    }
                }

                match download_file_direct(&session.client, &material.url, &mat_path, &cancel_token).await {
                    Ok(size) => {
                        total_bytes.fetch_add(size, Ordering::Relaxed);
                    }
                    Err(e) => {
                        tracing::error!(
                            "[afya] Material download failed for '{}': {}",
                            material.title,
                            e
                        );
                    }
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://alunos-internato.afya.com.br/"),
        cancel_token.clone(),
        None,
        8,
        false,
