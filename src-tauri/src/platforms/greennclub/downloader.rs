...
use super::api::{self, GreennCourse, GreennSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct GreennCourseDownloadProgress {
    pub course_id: i64,
...
pub async fn download_full_course(
    app: &tauri::AppHandle,
    session: &GreennSession,
    course: &GreennCourse,
    output_dir: &str,
    cancel_token: CancellationToken,
) -> anyhow::Result<()> {
    let mut modules = api::get_modules(session, course.id).await?;
    ...
    for module in &mut modules {
        let lessons = api::get_lessons(session, course.id, module.id).await?;
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            if let Some(ref content) = lesson.content {
                crate::core::course_utils::save_description(&lesson_dir, content, "html").await.ok();
            }

            let mut video_url: Option<String> = None;

            if let Some(ref source) = lesson.source {
                if let Some(ref mt) = lesson.media_type {
                    if mt.contains("panda") {
                        if let Some(_vid) = api::extract_panda_video_id(source) {
                            video_url = Some(source.clone());
                        }
                    }
                }
            ...
            if let Some(ref url) = video_url {
                let video_ext = if url.contains(".m3u8") {
                    "mp4"
                } else {
                    url.rsplit('.')
                        .next()
                ...
                if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                    ...
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        tracing::info!("[greenn] Skipping existing: {}", video_path);
                ...
                if url.contains(".m3u8") {
                    match download_hls_video(url, &video_path, &cancel_token).await {
                        Ok(size) => {
                            total_bytes.fetch_add(size, Ordering::Relaxed);
                        }
                        Err(e) => {
                            tracing::error!(
                                "[greenn] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(url, &lesson_dir, &cancel_token).await {
                        Ok(size) => {
                            total_bytes.fetch_add(size, Ordering::Relaxed);
                        }
                        Err(e) => {
                            tracing::error!(
                                "[greenn] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, url, &video_path, &cancel_token).await {
                                Ok(size) => {
                                    total_bytes.fetch_add(size, Ordering::Relaxed);
                                }
                                Err(e2) => {
                                    tracing::error!(
                                        "[greenn] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for attachment in &lesson.attachments {
                ...
                let att_name = filename::sanitize_path_component(&attachment.name);
                let att_path = format!("{}/{}", lesson_dir, att_name);
                ...
                match download_file_direct(&session.client, &attachment.url, &att_path, &cancel_token).await {
                    Ok(size) => {
                        total_bytes.fetch_add(size, Ordering::Relaxed);
                    }
                    Err(e) => {
                        tracing::error!(
                            "[greenn] Attachment download failed for '{}': {}",
                            attachment.name,
                            e
                        );
                    }
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        &ytdlp_path,
        url,
        out_dir,
        None,
...
async fn download_file_direct(
    ...
    crate::core::direct_downloader::download_direct(
        client,
        url,
        out,
        tx,
