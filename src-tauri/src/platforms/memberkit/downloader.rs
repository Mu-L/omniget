...
use super::api::{self, MemberkitCourse, MemberkitSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct MemberkitCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let modules = api::get_modules(session, &course.slug).await?;
    ...
    let _ = app.emit(
        "memberkit-download-progress",
        &MemberkitCourseDownloadProgress {
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lesson_detail(session, &lesson.url).await {
                Ok(d) => d,
                Err(e) => {
                    tracing::error!(
                        "[memberkit] Failed to get lesson detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                    ...
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        tracing::info!("[memberkit] Skipping existing: {}", video_path);
                        let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                        let _ = app.emit(
                            "memberkit-download-progress",
                            &MemberkitCourseDownloadProgress {
                ...
                match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[memberkit] yt-dlp download failed for '{}': {}, trying direct download",
                            lesson.name,
                            e
                        );
                        match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                            ...
                            Err(e2) => {
                                tracing::error!(
                                    "[memberkit] Direct download also failed for '{}': {}",
                                    lesson.name,
                                    e2
                                );
                            }
            ...
            for attachment in &detail.attachments {
                ...
                match download_file_direct(&session.client, &attachment.url, &att_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[memberkit] Attachment download failed for '{}': {}",
                            attachment.name,
                            e
                        );
                    }
