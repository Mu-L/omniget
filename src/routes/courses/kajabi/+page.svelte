<script lang="ts">
  ...
  let courses: KajabiCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("kajabi_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("kajabi_login_token", { token: token.trim() });
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("kajabi_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: KajabiCourse) {
    ...
    try {
      await invoke("start_kajabi_course_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("kajabi_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
