<script lang="ts">
  ...
  let courses: NutrorCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("nutror_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("nutror_login_token", {
        token: token.trim(),
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("nutror_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: NutrorCourse) {
    ...
    try {
      await invoke("start_nutror_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("nutror_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
...
{:else}
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="form">
        <label class="field">
          <span class="field-label">Bearer Token (newAuthToken cookie)</span>
          <textarea
        ...
        <label class="field">
          <span class="field-label">Refresh Token (optional)</span>
          <input
            type="text"
