<script lang="ts">
  ...
  let courses: AreaDeMembrosApiCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await pluginInvoke<string>("courses", "areademembros_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await pluginInvoke<string>("courses", "areademembros_login_token", { token: token.trim(), siteUrl: siteUrl.trim() });
  ...
  async function handleLogout() {
    try {
      await pluginInvoke("courses", "areademembros_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await pluginInvoke("courses", "areademembros_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: AreaDeMembrosApiCourse) {
    ...
    try {
      await pluginInvoke("courses", "start_areademembros_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await pluginInvoke("courses", "areademembros_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
...
{:else}
  <div class="page-center">
    <div class="login-card">
      <h2>Area de Membros</h2>

      <div class="form">
        <label class="field">
          <span class="field-label">Platform URL</span>
          <input
            type="url"
            placeholder="https://example.areademembros.com"
            bind:value={siteUrl}
            class="input"
            disabled={loading}
          />
