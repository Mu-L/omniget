<script lang="ts">
  ...
  function getCourseDownloadStatus(courseId: string): "idle" | "downloading" | "complete" | "error" {
    const item = downloads.get(Number(courseId) || 0);
    if (!item) return "idle";
    const s = item.status;
  ...
  function getCourseDownloadPercent(courseId: string): number {
    return downloads.get(Number(courseId) || 0)?.percent ?? 0;
  }

  async function downloadCourse(group: SkoolGroup) {
    const status = getCourseDownloadStatus(group.id);
  ...
  <div class="page-logged">
    ...
    {:else}
      ...
      <div class="courses-grid">
        {#each paginatedCourses as group (group.id)}
          <CourseCard
            name={group.name}
            price={`${group.num_courses} courses`}
            imageUrl={group.image_url ?? undefined}
            downloadStatus={getCourseDownloadStatus(group.id)}
            downloadPercent={getCourseDownloadPercent(group.id)}
            onDownload={() => downloadCourse(group)}
          />
        {/each}
      </div>
  ...
  <div class="page-center">
    <div class="login-card">
      ...
      {:else}
        <div class="form">
          <label class="field">
            <span class="field-label">skooltok Cookie</span>
            <textarea
              class="input token-textarea"
              placeholder="Paste your skooltok cookie value here..."
              bind:value={token}
              disabled={loading}
              rows="4"
            ></textarea>
