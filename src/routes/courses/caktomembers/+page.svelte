<script lang="ts">
  ...
  let courses: CaktoMembersCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("caktomembers_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("caktomembers_login_token", { token: token.trim() });
  ...
  async function handleLogout() {
    try {
      await invoke("caktomembers_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("caktomembers_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: CaktoMembersCourse) {
    ...
    try {
      await invoke("start_caktomembers_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("caktomembers_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
...
{:else}
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="form">
        <label class="field">
          <span class="field-label">Session Cookie (sessionid)</span>
          <textarea
